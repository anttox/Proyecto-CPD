...
def get_anomalies():
    ...
    return jsonify({
        'IsolationForest': anomalies_if.tolist() if isinstance(anomalies_if, np.ndarray) else anomalies_if,
        'OneClassSVM': anomalies_svm.tolist() if isinstance(anomalies_svm, np.ndarray) else anomalies_svm
    })
